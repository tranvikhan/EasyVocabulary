...
const RightBlock = (props) => {
  return (
    ...
    >
      <View style={tailwind("w-full p-4 bg-white rounded-xl ")}>
        ...
        >
          ...
          <TouchableOpacity
            activeOpacity={0.5}
            onPress={() => {
              Speech.speak(props.correct.en, { language: "en" });
              Speech.speak(props.correct.vi, { language: "vi" });
            }}
      ...
      <View style={tailwind("w-full p-4 bg-white rounded-xl mt-4")}>
        <Text
          style={tailwind(
            "font-bold text-xl px-2 pt-1 pb-4 border-b border-gray-200 text-gray-800"
          )}
        >
          Đáp án của bạn
        </Text>
        <Text
          multiline
          numberOfLines={2}
          style={tailwind("font-medium text-xl px-2 mb-2 text-red-500 mt-4")}
        >
          {props.userQestion}
        </Text>
      </View>
